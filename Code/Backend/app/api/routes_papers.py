...
def write_paper(request: PaperWriteRequest, db: Session = Depends(get_db)) -> PaperWriteResponse:
    ...
    """
    try:
        idea = get_idea(db, request.idea_id)
...
def read_paper(paper_id: str, db: Session = Depends(get_db)) -> Paper:
    """Get paper details.
...
def import_paper(request: dict, db: Session = Depends(get_db)) -> Paper:
    """Import a paper from ArXiv.

    Args:
        request: Dictionary containing arxiv_id.
        db: Database session dependency.
...
def download_paper(paper_id: str, db: Session = Depends(get_db)) -> Response:
